...
/* expander 화살표(>) 숨기기 — 주간/월별 전용 */
.no-arrow-expander details > summary::marker,
.no-arrow-expander details > summary::-webkit-details-marker {
    display: none !important;
    content: '' !important;
}

.no-arrow-expander details > summary > div > div:first-child {
    display: none !important;
}

.no-arrow-expander div[data-testid='stExpander'] details > summary svg {
    display: none !important;
}
...
.sidebar-preview-label .preview-date {
    font-size: 0.80rem;
    color: #9CA3AF;
    font-weight: 600;
}
...
def render_week_month_event(row, prefix=""):
    ...
    주간/월별 보기용 이벤트 카드.
    - 화살표(>) 없음
    - 박스 테두리·텍스트를 카테고리 컬러로
    - 펼쳤을 때 가로 2열 그리드로 핵심 정보 표시 후 액션 버튼
    """
    c = get_color(safe_str(row.get("Category", "기타")))
    time_text = safe_str(row.get("Time")) or ""
    cat_text  = safe_str(row.get("Category")) or "기타"
    subject   = compact_subject_text(row)
    is_cancel = safe_str(row.get("Status")) == "취소"

    # 헤더 라벨 (expander title)
    label = f"{time_text}  {cat_text}  {subject}"

    # expander 컨테이너 — 카테고리 컬러 주입
    card_style = f"""
    <style>
    /* prefix별 expander 스타일 오버라이드 */
    [data-testid='stExpander']:has(~ * [data-key='{prefix}_body_{row['ID']}']) details,
    div[data-key='{prefix}_wrap_{row["ID"]}'] details {{
    ...
    # 컨테이너로 감싸서 카테고리 컬러 적용
    container_id = f"wm_{prefix}_{row['ID']}"

    st.markdown(f"""
    <style>
    #{container_id} div[data-testid='stExpander'] details {{
        border-color: {c['line']} !important;
        background: {c['bg']} !important;
    }}
    #{container_id} div[data-testid='stExpander'] summary {{
        color: {c['text']} !important;
    }}
    #{container_id} div[data-testid='stExpander'] summary svg {{
        display: none !important;
    }}
    #{container_id} div[data-testid='stExpander'] summary p {{
        color: {c['text']} !important;
        font-weight: 700 !important;
    ...
    strike = "text-decoration:line-through;opacity:0.6;" if is_cancel else ""
    expander_label = f"{time_text} [{cat_text}] {subject}"

    with st.expander(expander_label, expanded=False):
        # 핵심 정보 — 가로 2열 그리드
        org     = esc(row.get("OrgName", ""))
        place   = esc(row.get("DetailPlace", ""))
        staff   = esc(row.get("Staff", ""))
        companion = esc(row.get("Companion", ""))
        purpose = esc(row.get("Purpose", ""))
        follow  = esc(row.get("FollowStatus", ""))
        ...
        <div style="margin-bottom:6px;">
            <span style="background:{c['soft']};color:{c['text']};border:1px solid {c['line']};
                  border-radius:999px;padding:3px 8px;font-size:0.70rem;font-weight:800;margin-right:4px;">
                {esc(cat_text)}
            </span>
            <span style="background:#F3F4F6;color:#374151;border:1px solid #D1D5DB;
                  border-radius:999px;padding:3px 8px;font-size:0.70rem;font-weight:700;margin-right:4px;">
                {status}
            </span>
            <span style="background:#F3F4F6;color:#374151;border:1px solid #D1D5DB;
                  border-radius:999px;padding:3px 8px;font-size:0.70rem;font-weight:700;margin-right:4px;">
            ...
            <span style="background:#F3F4F6;color:#374151;border:1px solid #D1D5DB;
                  border-radius:999px;padding:3px 8px;font-size:0.70rem;font-weight:700;">
                팔로우 {follow}
            </span>
        </div>
        <div class="detail-grid">
            <div class="detail-cell">
                ...
                <div class="detail-cell-value">{org if org != '-' else '—'}</div>
            </div>
            <div class="detail-cell">
                <div class="detail-cell-label">장소</div>
                <div class="detail-cell-value">{place if place != '-' else '—'}</div>
            </div>
            <div class="detail-cell">
                <div class="detail-cell-label">동행</div>
                <div class="detail-cell-value">{companion if companion != '-' else '—'}</div>
            </div>
            <div class="detail-cell">
                <div class="detail-cell-label">수행직원</div>
                <div class="detail-cell-value">{staff if staff != '-' else '—'}</div>
            </div>
            <div class="detail-cell" style="grid-column:span 2;">
                <div class="detail-cell-label">회의 목적</div>
                <div class="detail-cell-value">{purpose if purpose != '-' else '—'}</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
...
def render_form(mode="new", row_data=None):
    ...
    with st.form(f"form_{mode}", clear_on_submit=False):
        ...
        input_editor_name = r2c4.text_input(
            "작성자/수정자 이름", value=safe_str(row_data["UpdatedBy"]), placeholder="예: 홍길동"
        )
        ...
        if mode == "new":
            ...
            if submit_view or submit_continue:
                if not safe_str(input_subject):
                    ...
                    record = {
                        "ID": next_id(), "Date": str(input_date),
                        "Time": input_time.strftime("%H:%M"), "Category": input_category,
                        ...
                        "Companion": safe_str(input_companion), "Staff": safe_str(input_staff),
                        "Purpose": safe_str(input_purpose), "ActionPlan": safe_str(input_action),
                        "Memo": safe_str(input_memo), "Status": input_status, "Priority": input_priority,
                        "FollowOwner": safe_str(input_follow_owner),
                        "FollowTask": safe_str(input_follow_task), "FollowDue": follow_due_value,
                        "SharedNote": safe_str(input_shared_note), "FollowStatus": input_follow_status,
            ...
            if save_btn:
                if not safe_str(input_subject):
                    ...
                    record = {
                        "ID": row_data["ID"], "Date": str(input_date),
                        "Time": input_time.strftime("%H:%M"), "Category": input_category,
                        "Subject": safe_str(input_subject),
                        "PresidentAttend": "Y" if input_president_attend else "",
                        "OrgName": safe_str(input_org), "DetailPlace": safe_str(input_detail_place),
...
with sidebar_top:
    ...
    # ── 날짜를 아랫줄로 분리한 미리보기 expander ──
    preview_date_str = str(st.session_state.selected_date)
    with st.expander(f"📊 선택일 일정 미리보기\n{preview_date_str}", expanded=False):
...
# =========================================================
    ...
    search_text = fc1.text_input(
        "검색", value=st.session_state.search_text,
    ...
    with tabs[2]:
        ...
        month_year = mc1.selectbox(
            "년도", year_options,
            index=year_options.index(current_year), key="month_year_select"
        )
        month_month = mc2.selectbox(
            "월", list(range(1, 13)),
            index=st.session_state.selected_date.month - 1, key="month_month_select"
        )
        month_view_mode = mc3.radio(
            "보기 방식", ["캘린더형", "목록형"], horizontal=True, key="month_view_mode"
        )
